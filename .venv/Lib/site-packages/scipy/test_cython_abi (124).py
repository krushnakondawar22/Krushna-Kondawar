...
# Re-generate with:  python tools/generate_cython_abi_tests.py optimize
#
...
_sig_file = importlib.resources.files('scipy.optimize.tests').joinpath(
...
def test_cython_optimize_abi_stability():
    """No existing cython_optimize signature may change or disappear."""
    expected = _EXPECTED['scipy.optimize.cython_optimize._zeros']
    mod = importlib.import_module('scipy.optimize.cython_optimize._zeros')
    ...
    if errors:
        joined = '\n'.join(errors)
        pytest.fail(
            f"scipy.optimize.cython_optimize._zeros.__pyx_capi__ has {len(errors)} "
            f"ABI breakage(s):\n{joined}"
        )
